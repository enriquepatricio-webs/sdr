...
function Formulario() {
  ...
  return (
    <form onSubmit={entrar} className="w-full max-w-sm border border-linea bg-lienzo p-8">
      <p className="font-mono text-sm font-bold tracking-[0.18em] uppercase">SDR</p>
      <h1 className="mt-6 text-xl font-semibold">Entrar</h1>

      <label htmlFor="password" className="etiqueta mt-6 block">
        Contraseña
      </label>
      <input
        id="password"
      ...
      {error && (
        <p role="alert" className="mt-3 border-l-2 border-vivo bg-vivo-suave px-3 py-2 text-sm text-vivo">
          {error}
        </p>
      )}
