...
import { campaigns, leads, meetings } from "@/lib/db/schema";
...
export default async function PaginaReuniones() {
  // Las reuniones son de una empresa: son SUS clientes potenciales.
  ...
  const proximas = filas
    .filter((f) => f.reunion.startAt.getTime() >= ahora)
    .reverse();
            ...
            >
              <div className="w-44 shrink-0">
                <p className="font-mono text-sm">
                  {reunion.startAt.toLocaleDateString("es-ES", {
                    day: "2-digit",
                    month: "short",
                  })}
                  {" · "}
                  {reunion.startAt.toLocaleTimeString("es-ES", {
                    hour: "2-digit",
                    minute: "2-digit",
                  })}
                </p>
                <p className="text-[11px] text-tenue">
                  {Math.round(
                    (reunion.endAt.getTime() - reunion.startAt.getTime()) /
                      60000,
                  )}{" "}
                  min
                </p>
              ...
              <div className="min-w-0 flex-1">
                <Link
                  href={`/leads/${lead.id}`}
                  className="font-medium hover:text-ensayo"
                >
                  {lead.fullName}
                </Link>
              ...
              {reunion.meetUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="text-sm text-ensayo underline"
                >
                  Meet
                </a>
              )}
              <span
                className={`w-24 text-right font-mono text-[11px] uppercase ${COLOR[reunion.status]}`}
              >
      ...
      {filas.length === 0 ? (
        <div className="border border-dashed border-linea-fuerte p-10 text-center">
          <p className="text-sm text-tenue">
            Todavía no hay ninguna reunión agendada.
          </p>
        </div>
      ) : (
        <>
