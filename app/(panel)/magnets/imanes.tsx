...
type Cuenta = {
  ...
  displayName: string;
  status: string;
  /** null = sin autorizar en Meta. Sin token no puede leer ni un comentario. */
...
export function Imanes({
  ...
  /**
   * Leer los comentarios ahora, sin esperar a que Meta avise.
   *
   * En marcha normal no hace falta: Meta avisa y el recurso sale en segundos.
   * Sirve para los comentarios de ANTES de conectar la cuenta, que nadie va a
   * anunciar nunca, y para comprobar que un imán recién creado apunta a la
   * publicación correcta.
   *
   * Siempre en ensayo primero: esto escribe a personas reales y no hay forma de
   * recuperar un mensaje enviado.
   */
  async function revisar(iman: Iman, ensayo: boolean) {
    const r = await llamar(`/api/magnets/${iman.id}/comentarios`, "POST", {
      ensayo,
      maximo: 10,
    });
    if (!r) return;
    if (!r.hechos?.length) {
      return setAviso(
        `${r.comentariosLeidos} comentarios en ${r.cuenta}, ${r.conLaClave} con la palabra, ninguno nuevo.`,
      );
    }
    const quienes = r.hechos
      .map((h: { usuario: string }) => "@" + h.usuario)
      .join(", ");
    setAviso(
      ensayo
        ? `Se escribiría a ${quienes}. Pulsa «Escribirles» para enviarlo de verdad.`
        : `Enviado a ${quienes}.`,
    );
  }
    ...
    <div className="space-y-6">
      ...
      {!autopilot && (
        <p className="border border-ensayo bg-ensayo-suave px-3 py-2 text-xs text-ensayo">
          El autopiloto de {empresa} está apagado: cada ciclo deja los mensajes
          en borrador y no sale nada.
        </p>
      )}
      {error && (
        <p className="border border-vivo bg-vivo-suave px-3 py-2 text-xs text-vivo">
          {error}
        </p>
      )}
      {aviso && (
        <p className="border border-linea bg-lienzo px-3 py-2 text-xs text-apagado">
          {aviso}
        </p>
      )}
      ...
      {abierto && (
        <form
          onSubmit={crear}
          className="grid gap-3 border border-linea bg-lienzo p-4 sm:grid-cols-2"
        >
          ...
          <label className="space-y-1">
            ...
            <select name="accountId" required className={control}>
              {/* Sin autorizar no se puede elegir: el imán no podría leer ni un
                  comentario, y nacería sin decir por qué no hace nada. */}
              {cuentas.map((c) => (
                <option key={c.id} value={c.id} disabled={!c.autorizada}>
                  {c.displayName}
                  {!c.autorizada
                    ? " — sin autorizar en Instagram"
                    : c.status !== "active"
                      ? ` (${c.status})`
                      : ""}
                </option>
            ...
            {cuentas.every((c) => !c.autorizada) && (
              <p className="text-sm text-aviso">
                Ninguna cuenta está autorizada todavía. Hazlo en{" "}
                <a className="underline" href="/empresa">
                  Empresa
                </a>
                .
              </p>
              ...
              <div className="flex flex-wrap items-center gap-4 border-b border-linea px-4 py-3">
                <div className="min-w-0 flex-1">
                  <p className="font-medium">
                    {iman.name}{" "}
                    <span className="font-mono text-xs text-ensayo">
                      “{iman.keyword}”
                    </span>
                  </p>
                ...
                <button
                  ...
                  onClick={() =>
                    llamar(`/api/magnets/${iman.id}`, "PATCH", {
                      active: !iman.active,
                    })
                  }
                ...
                <button
                  type="button"
                  disabled={ocupado !== null}
                  onClick={() => revisar(iman, true)}
                  className="border border-ensayo px-2 py-1 text-xs text-ensayo hover:bg-ensayo-suave disabled:opacity-40"
                >
                  {ocupado ? "Mirando…" : "Ver comentarios sin responder"}
                </button>
                <button
                  type="button"
                  disabled={ocupado !== null || !iman.active}
                  onClick={() => revisar(iman, false)}
                  className="border border-vivo px-2 py-1 text-xs text-vivo hover:bg-vivo-suave disabled:opacity-40"
                >
                  Escribirles
                </button>
                <button
