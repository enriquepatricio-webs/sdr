...
export async function proxy(request: NextRequest) {
  ...
  const hasSession = await isValidSession(request.cookies.get(SESSION_COOKIE)?.value)
  if (hasSession) return NextResponse.next()

  if (isValidApiKey(request.headers.get('x-api-key'))) return NextResponse.next()

  // La API contesta 401 en JSON; a n8n no le sirve una redirección a un login.
  if (pathname.startsWith('/api/')) {
    return NextResponse.json(
      { error: 'No autorizado. Falta la cookie de sesión o la cabecera x-api-key.' },
      { status: 401 },
    )
  }
