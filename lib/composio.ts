...
export async function ejecutar<T>(
  ...
  opciones: { userId?: string; connectedAccountId?: string; version?: string } = {},
): Promise<T> {
  const res = await fetch(`${BASE}/tools/execute/${encodeURIComponent(slug)}`, {
    ...
    body: JSON.stringify({
      ...
      ...(opciones.connectedAccountId ? { connected_account_id: opciones.connectedAccountId } : {}),
    }),
    cache: 'no-store',
  })
  ...
  if (!res.ok) {
    throw new ComposioError(`Composio respondió ${res.status}: ${texto.slice(0, 300)}`, res.status, texto)
  }
  ...
  try {
    ...
    throw new ComposioError('Composio devolvió algo que no es JSON', res.status, texto)
  }
...
export function calcularHuecos(
  ...
  const { timezone, duration_min, min_notice_hours, buffer_min, lookahead_days, working_hours } =
    reglas
  ...
  for (let salto = 0; salto <= lookahead_days && huecos.length < reglas.max_slots_offered; salto++) {
    ...
    for (let t = abre.getTime(); t + duration_min * 60_000 <= cierra.getTime(); t += PASO_MINUTOS * 60_000) {
...
function extraerOcupado(data: RespuestaHuecos): Intervalo[] {
  ...
  return Object.values(calendarios)
    ...
    .filter((i) => !Number.isNaN(i.inicio.getTime()) && !Number.isNaN(i.fin.getTime()))
}
...
export async function consultarDisponibilidad(
  reglas: BookingRules,
  opciones: { userId?: string; connectedAccountId?: string; calendarId?: string } = {},
  ahora: Date = new Date(),
): Promise<Intervalo[]> {
  const hasta = new Date(ahora.getTime() + reglas.lookahead_days * 24 * 3600_000)
...
export type EventoCreado = { id: string; htmlLink?: string; hangoutLink?: string; meetUrl?: string }
...
export async function agendarReunion(
  ...
  const data = await ejecutar<EventoCreado>(
    HERRAMIENTAS.crearEvento,
    {
      ...
      event_duration_minutes: Math.round((datos.fin.getTime() - datos.inicio.getTime()) / 60000),
