...
await prueba('abrir chat va como multipart y SIN Content-Type manual', async () => {
  const c = simular({ chat_id: 'c1', message_id: 'm1' })
  await iniciarChat({ accountId: 'a1', attendeeId: 'ACo123', texto: 'hola' })
  const headers = c[0].init.headers as Record<string, string>
...
await prueba('el InMail solo se pide cuando se pide', async () => {
  ...
  await iniciarChat({ accountId: 'a1', attendeeId: 'ACo1', texto: 'x', inmail: true })
...
await prueba('un error de Unipile se propaga con su estado', async () => {
  ...
  await assert.rejects(() => iniciarChat({ accountId: 'a1', attendeeId: 'x', texto: 'y' }), /422/)
})
...
await prueba('NUESTRO propio mensaje se reconoce como eco y no como respuesta', () => {
...
await prueba('otros eventos se ignoran', () => {
  assert.equal(interpretarWebhook({ event: 'message_read', chat_id: 'c', message_id: 'm' }), null)
})

await prueba('un webhook sin chat_id o message_id se descarta', () => {
  assert.equal(interpretarWebhook({ event: 'message_received', message: 'x' }), null)
})
...
await prueba('respeta la antelación mínima', () => {
  ...
  assert.ok(h[0].inicio.getTime() >= minimo, `propuso ${h[0].inicio.toISOString()}, demasiado pronto`)
})
...
await prueba('nunca propone fuera del horario laboral', () => {
  ...
  for (const x of h) {
    ...
    assert.ok(min + REGLAS.duration_min <= 18 * 60 + 30, `termina después de cerrar`)
  }
})

await prueba('nunca propone sábado ni domingo', () => {
  const h = calcularHuecos([], { ...REGLAS, max_slots_offered: 5, lookahead_days: 14 }, LUNES)
  for (const x of h) {
    const dow = new Intl.DateTimeFormat('en-US', { timeZone: 'Europe/Madrid', weekday: 'short' })
      .format(x.inicio)
...
await prueba('respeta el colchón alrededor de lo que ya hay', () => {
  ...
  const ocupado = [
    { inicio: new Date('2026-08-17T07:30:00Z'), fin: new Date('2026-08-17T14:00:00Z') },
    { inicio: new Date('2026-08-17T14:30:00Z'), fin: new Date('2026-08-17T16:30:00Z') },
  ]
  const h = calcularHuecos(ocupado, { ...REGLAS, max_slots_offered: 5 }, LUNES)
  const elLunes = h.filter((x) => x.inicio.toISOString().startsWith('2026-08-17'))
  assert.equal(elLunes.length, 0, 'metió una reunión en un agujero sin colchón')
})
...
await prueba('consultarDisponibilidad pide el rango correcto', async () => {
  const c = simular({ successful: true, data: { calendars: { primary: { busy: [] } } } })
...
await prueba('un successful:false lanza aunque el HTTP sea 200', async () => {
  ...
  await assert.rejects(() => consultarDisponibilidad(REGLAS, {}, LUNES), /token caducado/)
})
...
await prueba('los huecos ocupados que devuelve Composio se respetan', async () => {
  simular({
    successful: true,
    data: {
      calendars: {
        primary: {
          busy: [{ start: '2026-08-17T07:00:00Z', end: '2026-08-17T20:00:00Z' }],
        },
      },
    },
  })
...
await prueba('el hueco se describe en la zona del prospecto y en español', () => {
  const texto = describirHueco(
    { inicio: new Date('2026-08-17T08:00:00Z'), fin: new Date('2026-08-17T08:30:00Z') },
    'Europe/Madrid',
  )
