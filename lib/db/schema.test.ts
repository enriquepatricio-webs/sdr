...
async function main() {
  ...
  await ok(
    "15 tablas: las 9 del spec, prospección, empresas, imanes y usuarios",
    async () =>
      assert.deepEqual(
        [...names].sort(),
        [
  ...
  await ok(
    "leads_next_action_at_idx es parcial y excluye los 5 estados terminales",
    async () => {
      ...
      for (const terminal of [
        ...
        assert.ok(
          def.includes(terminal),
          `el predicado no excluye '${terminal}': ${def}`,
        );
      }
    },
  );
  ...
  await rejects(
    db,
    "daily_limit 0 se rechaza",
    `update accounts set daily_limit = 0`,
  );
  await rejects(
    db,
  ...
  await ok("score null es válido (lead aún sin cualificar)", () =>
    db.exec(
      `update leads set score = null where id='44444444-4444-4444-4444-444444444444'`,
    ),
  );
  ...
  await ok("deja exactamente 1 playbook y está activo", async () => {
    const rows = await seedDb
      .select({
        ...
        criteria: playbooks.qualificationCriteria,
      })
      .from(playbooks);
  ...
  await ok(
    "el autopiloto arranca APAGADO (restricción no negociable)",
    async () => {
      const rows = await seedDb
        .select()
        .from(settings)
  ...
  await ok(
    `ningún tope diario sembrado supera ${MAX_DAILY_LIMIT}`,
    async () => {
      const [acc] = await seedDb
        .select({ limit: accounts.dailyLimit })
        .from(accounts);
      const [camp] = await seedDb
        .select({ cap: campaigns.dailyCap })
        .from(campaigns);
  ...
  await ok("el umbral para agendar es alcanzable pero no trivial", async () => {
    const [pb] = await seedDb
      .select({
        criteria: playbooks.qualificationCriteria,
        rules: playbooks.bookingRules,
      })
      .from(playbooks);
    const weights = (pb.criteria ?? [])
      .map((c) => c.weight)
      .sort((a, b) => b - a);
    ...
    assert.ok(
      threshold < 100,
      "el umbral exige el 100%: nadie agendaría nunca",
    );
    assert.ok(
      threshold > weights[0],
      `basta el criterio de más peso (${weights[0]}) para agendar`,
    );
  });

  await ok("el system prompt recoge las 5 reglas duras del spec", async () => {
    const [pb] = await seedDb
      .select({ prompt: playbooks.systemPrompt })
      .from(playbooks);
    for (const rule of [
      ...
      assert.ok(
        pb.prompt.includes(rule),
        `el system prompt no menciona "${rule}"`,
      );
    }
  });
  ...
  await ok(
    "touches guarda la cuenta emisora y sobrevive a reasignar la campaña",
    async () => {
      const [lead] = (
        await fixPg.query<{ id: string }>(
          `select id from leads where full_name='Carla'`,
        )
      ).rows;
      ...
      assert.equal(
        r.rows[0].n,
        1,
        "el toque se ha ido con la campaña en vez de quedarse en su cuenta",
      );
    },
  );
  ...
  await ok(
    "el índice parcial cubre exactamente TERMINAL_LEAD_STATUSES",
    async () => {
      const def =
        (
  ...
  await ok(
    "sin tope horario también vale (LinkedIn y email no lo necesitan)",
    () =>
  ...
  await ok(
    "una búsqueda de prospectos se registra con su actor y su entrada",
    () =>
  ...
  await ok(
    "se puede activar cualquier versión, venga antes o después de la activa",
    async () => {
      ...
      commit;`);
        ...
        assert.equal(
          r.rows.length,
          1,
          `quedaron ${r.rows.length} playbooks activos al activar la v${v}`,
        );
  ...
  await ok(
    "la versión de una sola sentencia SÍ falla (por eso no se usa)",
    async () => {
      let fallo = false;
      try {
        await pbPg.exec(
  ...
  await ok(
    "activar el de una empresa deja intacto el playbook global",
    async () => {
  ...
  await ok(
    '"no_interesado" es terminal: la puerta de salida tiene que bloquearlo',
    async () => {
      assert.ok(
      ...
      assert.ok(
        (TERMINAL_LEAD_STATUSES as readonly string[]).includes(
          "revision_humana",
        ),
      );
    },
  ...
  await ok(
    "entregar un lead lo reserva: la segunda llamada ya no lo ve",
    async () => {
      ...
      assert.equal(
        primera.rows.length,
        2,
        "la primera llamada no entregó los dos leads que tocaban",
      );
      ...
      assert.equal(
        tercera.rows.length,
        0,
        "quedaban leads sin reservar que ya se habían entregado",
      );
    },
  );
